...
export default function AdminProductsDetailsPage() {
  const [mainImage, setMainImage] = useState(
    "https://m.media-amazon.com/images/I/612JvadSUzL._SY741_.jpg"
  );

  const [notificationProductView, setNotificationProductView] = useState(null);
  const { id } = useParams();
  const { notifications } = useSelector((state) => state.UserSliceProvider);
console.log(id)
  ...
  useEffect(() => {
      ...
      if (Array.isArray(notifications) && notifications.length > 0) {
          const foundNotifications = notifications.find((notification) => 
              notification.products && notification.products.productId===id
          )

          if (foundNotifications) {
              setNotificationProductView(foundNotifications.products)
              setMainImage(foundNotifications.products.img1);
              console.log("Found Products:",foundNotifications.products)
          } else {
              setNotificationProductView(null)
  ...
  if (!notificationProductView) {
    return <div className="pt-[500px]">Loading...</div>; 
  }
    const product = notificationProductView; 
